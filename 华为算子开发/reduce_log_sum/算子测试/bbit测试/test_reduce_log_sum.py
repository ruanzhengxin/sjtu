...
def generate_model(input_shape, output_shape, model_save_path, **kwargs):
    ...
    model_def = onnx.helper.make_model(graph_def,
                                       producer_name='reduce_log_sum')
    model_def.opset_import[0].version = 11
    onnx.checker.check_model(model_def)

    onnx.save(model_def, model_save_path)

def reduce_log_sum(input_shape, output_shape, axes=None, keepdims=1, input_dtype=np.float32):
    input_data = np.random.random(input_shape).astype(input_dtype)*10+1
    print(input_data)
    if axes is None:
        expect_data = np.sum(input_data, axis=axes, keepdims=keepdims == 1)
    else:
        expect_data = np.sum(input_data, axis=tuple(axes), keepdims=keepdims == 1)
    expect_data = np.log(expect_data)
    print(expect_data)
    save_data("reduce_log_sum", input_shape, output_shape, input_data, expect_data)



if __name__ == "__main__":

    op_name = "reduce_log_sum"
    input_shape = [8, 16, 4]
    output_shape = [8, 16, 1]
    axes = [2]
    keepdims = 1
    ...
    reduce_log_sum(input_shape, output_shape, axes=axes, keepdims=keepdims, input_dtype=np.float32)
